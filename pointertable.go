...
var fiTable = map[uint32]dokan.File{}
var fiIdx uint32
...
func fsTableStore(fs dokan.FileSystem, ec chan error) uint32 {
	fsTableLock.Lock()
...
func fsTableGet(slot uint32) dokan.FileSystem {
	fsTableLock.Lock()
	defer fsTableLock.Unlock()
...
func fiTableStoreFile(global uint32, fi dokan.File) uint32 {
	fsTableLock.Lock()
...
func fiTableGetFile(file uint32) dokan.File {
	fiTableLock.Lock()
	var fi = fiTable[file]
	fiTableLock.Unlock()
	return fi
